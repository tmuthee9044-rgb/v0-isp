...
ALTER TABLE customers ADD COLUMN IF NOT EXISTS prepaid_remains_days VARCHAR(255);
...
ALTER TABLE customers ADD COLUMN IF NOT EXISTS gdpr_agreed VARCHAR(100);
...
ALTER TABLE customers ADD COLUMN IF NOT EXISTS login VARCHAR(255);
...
ALTER TABLE customers ADD COLUMN IF NOT EXISTS added_by_id VARCHAR(100);
...
ALTER TABLE customers ADD COLUMN IF NOT EXISTS last_online DATE;
...
-- Create indexes for servers table (rule 6 - fast page loads) - only if table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'servers') THEN
        ...
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'servers' AND column_name = 'location_id') THEN
            CREATE INDEX IF NOT EXISTS idx_servers_location_id ON servers(location_id);
        END IF;
        
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'servers' AND column_name = 'monitoring_enabled') THEN
            CREATE INDEX IF NOT EXISTS idx_servers_monitoring ON servers(monitoring_enabled);
        END IF;
        
        IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'servers' AND column_name = 'last_seen') THEN
...
-- Create tax_records indexes only if table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tax_records') THEN
...
-- Create audit_logs indexes only if table exists
DO $$
BEGIN
...
-- Only create tax_records sequence if table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'tax_records') THEN
...
-- Only create audit_logs sequence if table exists
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_logs') THEN
