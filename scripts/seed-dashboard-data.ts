...
// This script populates the database with realistic sample data for dashboard display
// Uses getSql() for Rule 4 compliance (dual database support)
...
async function seedDashboardData() {
  ...
  try {
    // 1. Seed Customers (50 active customers)
    ...
    for (const plan of servicePlans) {
      await sql`
        INSERT INTO service_plans (
          ...
          ${plan.name},
          ${'Reliable internet service for your needs'},
          ${plan.download},
          ${plan.upload},
          ${plan.price},
          'active'
    ...
    // 3. Seed Customer Services (40 active services)
    console.log('Seeding customer services...')
    ...
    // 4. Seed Invoices (monthly invoices for active services)
    ...
    // 5. Seed Payments (payments for paid invoices)
    ...
    // 7. Seed Network Devices
    console.log('Seeding network devices...')
    await sql`
      INSERT INTO network_devices (name, type, ip_address, location, status)
      VALUES 
    ...
    console.log('- Active Services: 40')
    console.log('- Invoices: ~120')
    console.log('- Payments: ~80')
    console.log('- Support Tickets: 15')
    console.log('- Network Devices: 3')
